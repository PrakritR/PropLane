...
| Secondary utility in the same strip | `PortalIconAction` (icon; tooltip + `aria-label`) |
...
## Title / card header utilities

Secondary utilities in a title or card header are icons (`PortalIconAction`).
The word lives in the tooltip and `aria-label` only.

Labeled text is only for a data-commit (**Save**) or a destructive confirm.
Do not put Share, Copy, Filter, Settings, Download, Edit, or Delete on a
labeled pill in that chrome.
...
A dialog launched from a ⋯ item must not live inside the menu: the menu backdrop (`z-[10050]`) frosts any modal stacked beneath it (z-70–90). Call `RecordActionCloseContext` first, then open the dialog from a host mounted outside the menu — Share uses `PortalRecordShareHost`, which `PortalRecordListSurface` mounts once per list (`portal-record-share-host.tsx`); `PortalRecordShareLinkButton` renders as a menu item automatically under `RecordActionItemsContext`. Guard: `tests/unit/portal-record-share-menu.test.tsx`.

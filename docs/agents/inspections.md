...
Uploads use `useNativeCamera`; PDF downloads use `downloadBlobFile`, giving Capacitor its native picker and share/save flow. PDFs are generated from the saved server snapshot, contain observations, photos, comparison baseline, history and acknowledgment, and never accept client-supplied storage paths or document bodies. A room section holds a single item named after the section, so the export prints that name once rather than twice. When a room-scoped report's baseline is an original 15-area document its item ids no longer match, so the export appends the preserved PRIVATE-ROOM baseline observations and photos as their own section — baseline evidence is never dropped, and shared property areas are never imported into an assigned-room inspection.

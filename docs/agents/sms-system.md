...
- A **blind relay**. `handleManagerReplyInbound` forwarded whatever the manager
  typed to their most-recently-active resident thread. Personal-cell forwarding
  is off in the managed regime (`forwardResidentInboundToManagerCell` returns
  false), so the manager had never been shown the thread they were replying
  into — the target was effectively a guess. Reaching a resident is now an
  explicit, named proposal: "text Jane that the plumber comes Tuesday" previews
  the recipient and the exact body, and `YES` sends it.

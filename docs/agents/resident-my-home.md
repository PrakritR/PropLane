...
Placement's default content is a three-row move-in checklist (C130) — lease signed, move-in charges paid, move-in inspection photographed — each linking to that fact's own tab (Lease, Payments, Inspections). It reads no new table: `leaseSigned` is the caller's already-resolved access check, charges come from the same client sync every Payments read uses (`isPendingUpfrontMoveInCharge` / `isUnpaidHouseholdCharge`), and the inspection fact is the resident's own primary move-in report's photo count (`loadInspectionList` + `pickPrimaryInspectionReport`). A row reads "Checking…" until its own source resolves, never a false "Not yet".

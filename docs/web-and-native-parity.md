...
4. **Photo upload** — Use `useNativeCamera()` so the app gets the native picker and the web keeps `<input type="file">`.
   **Lease and application PDFs** use the shared file input and server import routes in the WebView.
...
7. **Communication credit top-ups** — Balance and usage are shared across web and native, but Stripe credit-pack checkout is web-only: the app shows the balance with no checkout or external-purchase link (Apple consumable products / RevenueCat fulfillment are not configured). Contract: [`docs/agents/comms-billing.md`](agents/comms-billing.md).
8. **Tests** — Run `npm run test:unit` — `tests/unit/platform-parity.test.ts` and `tests/unit/portal-nav-order.test.ts` guard registries and web/native nav parity.
9. **Native shell only** — Plugins, icons, `Info.plist` permissions (incl. `@revenuecat/purchases-capacitor`): `npx cap sync` and rebuild in Xcode / Android Studio.

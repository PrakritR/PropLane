...
The 21 mirror records retain all original bundle records. Fifteen have exact normalized statement lineage. The remaining six are catalog-certified: co-manager permissions (COMMENT representation only), two reminder constraints, sales provenance, utility allocations, and shared-room capacity. `manager_assistant_emails_mailbox_local` maps to exact `20260906211554_manager_assistant_emails` lineage. Do not replay co-manager or expiry backfills.

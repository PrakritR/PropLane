...
[Aggregate machine-readable evidence](2026-09-05-production-encryption-evidence.json) contains counts and ciphertext archive hashes only. Four encrypted archives were also copied to the primary workspace’s gitignored `.staging-prod-sync/security-backups/production-20260905/` directory with private permissions and matching checksums, so they do not depend on temporary worktree retention. Original object names, customer values and keys remain outside Git. Archive authentication using the configured key is not an independent key-recovery or database/object restore drill.

## Strict-read cutover

All four strict flags were configured for staging after its zero-plaintext backfills. The strict staging deployment `dpl_B4C2FCgGz52keVqTbDenYT3r6o5C` is READY at revision `108b6533`; its encryption and upload-component sources match the reviewed revision. [Strict-mode staging browser/HTTP QA](2026-09-05-staging-strict-security-qa.json) passed, including deliberate plaintext applicant rejection, authorized encrypted roundtrips and synthetic cleanup. All four production flags were then configured true, and production redeploy `dpl_EPC5MjAjdtJS2Bw5mzxgDYM7tZsz` of the current `108b6533` release is **READY**, serving `prop-lane.space`. Final live checks returned sign-in 200 and unauthenticated applications 401 with CSP/HSTS. Vercel classifies these production variables as sensitive, so readback redacts their values; configuration evidence is the successful true-valued writes before this redeploy plus presence/type checks. No synthetic plaintext record was inserted in production.

...
Each manager gets a Stripe Connect account stored in `profiles.stripe_connect_account_id`.
...
### Identity verification (PLAN-0920-1500 Part C) — PropLane's own form, no hosted or embedded Stripe onboarding for new accounts

A **new** manager or vendor Connect account is created with
`controller: { requirement_collection: "application", stripe_dashboard: { type: "none" }, fees: { payer: "application" }, losses: { payments: "application" } }`
(`AXIS_CONNECT_CONTROLLER` in `src/lib/stripe-connect.ts`) — PropLane owns
collecting whatever Stripe's Accounts API says is due, in PropLane's own UI.
`src/lib/stripe-connect-identity.server.ts` turns
`account.requirements.currently_due`/`past_due` into a typed field list
(`getIdentityRequirements`) and applies a submission (`submitIdentity`) via
`GET`/`POST /api/stripe/connect/identity` (vendor twin under
`/api/vendor/stripe-connect/identity`); a document upload proxies straight to
Stripe Files through `POST …/identity/document` and returns only a file id.
SSN / ID number / tax ID are tokenized in the browser with Stripe.js
(`stripe.createToken('account', …)` for the individual/business bundle,
`createToken('person', …)` for an additional company representative) —
PropLane's server only ever forwards the resulting token id to Stripe and
never logs or stores the value itself. The verify sheet
(`src/components/portal/payout-verify-sheet.tsx`) renders that field list; a
requirement key it does not recognize, or a legacy account, falls back to
Stripe's embedded `account_onboarding` component in the same sheet (see
below) rather than blocking the manager.

**A legacy `stripe_dashboard.type: "express"` account is untouched** and
keeps finishing any remaining requirements through Stripe's embedded
component (`isApplicationCollected()` in `src/lib/stripe-connect.ts` tells the
two apart at runtime). For those accounts only, the onboard routes
(`src/app/api/stripe/connect/onboard/route.ts` and its vendor twin) still
ensure the Connect account exists and report `mode: "embedded"`; the client
mounts Stripe's `account_onboarding` / `account_management` embedded
components inside PropLane's own modal chrome using a client secret from
`POST /api/stripe/connect/account-session` (`src/lib/stripe-connect-embedded.ts`).
For an **application-collected** (new) account, the onboard routes instead
...
reads Stripe fresh; the cache exists only so a settings surface can show a
status without an extra Stripe round trip. Applied to **dev/test only** so
far — apply to staging/production before this ships live.

**Test-mode identity values** (Stripe test mode only): SSN `000000000` (last 4
`0000`), DOB `1901-01-01`, and any address is accepted by using
`address_full_match` as the address line — see
...
2. **Verify identity** in PropLane's own form (a new account) or Stripe's embedded onboarding (a legacy account) — see above.
...
| `onboard` route 409s `USE_IN_APP_IDENTITY` | Expected for a new account — open the Verify-identity sheet (`/api/stripe/connect/identity`), not the embedded component |
| Verify sheet falls back to the embedded component | `fallbackToEmbedded: true` from `getIdentityRequirements` — Stripe is asking for a requirement key this form doesn't map yet; check `currentlyDue` in the response |

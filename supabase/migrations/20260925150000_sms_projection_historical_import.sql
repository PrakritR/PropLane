-- Import historical originals whose retired/proxy line has no manager work-number
-- epoch. The service-only RPC reads the exact persisted source row itself: no
-- caller-supplied body, phone, owner, role, or timestamp can enter the model.
-- This does not enable a retired transport or make an unresolved thread sendable.
alter table public.sms_projection_turns add column if not exists provider_sid text;
update public.sms_projection_turns set provider_sid=source_event_id
  where provider_sid is null and source_namespace like 'twilio:%' and source_event_id like 'SM%';
create unique index if not exists sms_projection_turns_owner_provider_sid_uniq
  on public.sms_projection_turns(owner_manager_user_id,provider_sid) where provider_sid is not null;
...
begin
  ...
  if v_sid is not null and exists(select 1 from public.sms_projection_turns t
      where t.owner_manager_user_id=v_owner and t.provider_sid=v_sid) then
  ...
  insert into public.sms_projection_conversations(
    ...
    v_owner,v_role,v_line_id,v_key,'unresolved',v_user,v_phone,coalesce(v_line_phone,'historical:unplaced'),
    v_legacy_key,jsonb_build_object('historical',true,'sendDisabled',true,'sourceTable',p_source_table)
  ) on conflict(owner_manager_user_id,counterparty_role,work_line_id,identity_key)
  ...
  foreach v_alias_kind in array array['legacy_key','legacy_thread'] loop
    ...
    if v_alias_value is not null and v_alias_value<>'' then
      insert into public.sms_projection_aliases(owner_manager_user_id,counterparty_role,work_line_id,alias_kind,alias_value,conversation_id,source_owner_id)
      ...
      if v_existing is distinct from v_conversation.id then raise exception 'historical alias collision' using errcode='23505'; end if;
    end if;
  end loop;

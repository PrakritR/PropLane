...
declare
  ...
  bill record; claimed bigint; proposed bigint; new_items jsonb;
...
begin
  ...
  new_items := p_itemization;
  if p_history is null then
    if exists(select 1 from jsonb_array_elements(new_items) x where coalesce(x->>'kind','deduction') <> 'deduction' or (x->>'amountCents')::bigint <= 0) then raise exception 'Only new positive deductions belong in itemization'; end if;
    select coalesce(sum((x->>'amountCents')::bigint),0) into proposed from jsonb_array_elements(new_items) x;
    if proposed <> p_withhold and jsonb_array_length(new_items)>0 then raise exception 'Itemization must equal this disposition deduction'; end if;
    if p_withhold>0 and jsonb_array_length(new_items)=0 then
      new_items := jsonb_build_array(jsonb_build_object('kind','deduction','label',coalesce(p_memo,'Withheld'),'amountCents',p_withhold,'date',p_date));
    end if;
    -- Lock shared source bills in deterministic order. A second resident's
    -- disposition sees the first committed claim before checking remaining cost.
    for bill in select b.* from public.manager_bills b where b.id in
      (select (x->'evidence'->>'billId')::uuid from jsonb_array_elements(new_items) x where x->'evidence'->>'billId' is not null)
      order by b.id for update loop
      if bill.manager_user_id <> p_owner or bill.property_id is distinct from d.property_id or bill.status not in ('approved','scheduled','paid') then raise exception 'Supporting bill unavailable'; end if;
      select coalesce(sum((x->>'amountCents')::bigint),0) into claimed
        from public.security_deposit_ledger sd cross join lateral jsonb_array_elements(sd.itemization) x
        where sd.manager_user_id=p_owner and x->'evidence'->>'billId'=bill.id::text;
      select sum((x->>'amountCents')::bigint) into proposed from jsonb_array_elements(new_items) x where x->'evidence'->>'billId'=bill.id::text;
      if claimed+proposed>bill.amount_cents then raise exception 'Supporting bill already allocated or deduction exceeds remaining cost'; end if;
    end loop;
    if exists(select 1 from jsonb_array_elements(new_items) x where x->'evidence'->>'billId' is not null and not exists(select 1 from public.manager_bills b where b.id=(x->'evidence'->>'billId')::uuid and b.manager_user_id=p_owner)) then raise exception 'Supporting bill unavailable'; end if;
  end if;
  ...
  if p_history is null then
    select coalesce(jsonb_agg(x || jsonb_build_object('kind','deduction','date',p_date,'journalId',last_j)), '[]'::jsonb) into new_items from jsonb_array_elements(new_items) x;
    if p_refund>0 then new_items := new_items || jsonb_build_array(jsonb_build_object('kind','refund','label',coalesce(p_memo,'Refund disposition'),'amountCents',p_refund,'date',p_date,'journalId',last_j)); end if;
    new_items := coalesce(d.itemization,'[]'::jsonb) || new_items;
  end if;
  update public.security_deposit_ledger set amount_held_cents=remaining,status=next_status,
    disposition_type=p_type,disposition_date=p_date,itemization=new_items,

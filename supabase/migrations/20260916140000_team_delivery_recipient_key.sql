-- WS5 (team-comms) follow-up: a team delivery's `recipient_key` is
-- `team:<manager>` rather than the bare manager id.
--
-- `emitActionEvent` throttles by `recipient_key` (a 10-minute window, four
-- events and the copy becomes a digest). A team delivery keyed on the bare
-- manager id shared that window with the manager's OWN `manager` delivery,
-- so every team-visible event counted twice against the owner and their copy
-- flipped to "Several updates were recorded" after two events instead of
-- four. The bus now keys team rows `team:<manager>` (see `teamRecipientKey`
-- in `src/lib/action-events.server.ts`); this re-issues the durable lease
-- RPC so its identity check on a `team` delivery matches that shape. The
-- body is otherwise identical to `20260916130000_team_comms_action_events.sql`.
    ...
    for v_delivery in select value from jsonb_array_elements(p_event->'deliveries') loop
      if v_delivery->>'audience' is null
         ...
         or (v_delivery->>'audience' = 'team' and
             (v_delivery->>'recipientUserId' is distinct from v_manager::text
              or v_delivery->>'recipientKey' is distinct from 'team:' || v_manager::text))
...
comment on function public.persist_lease_with_action_event(jsonb,timestamptz,jsonb) is
  'Service-only CAS lease persistence plus durable action-event enqueue. Performs no external delivery. WS5: audience also accepts team (recipientUserId = the manager, recipientKey = team:<manager>).';

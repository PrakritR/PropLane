-- WS5 (team-comms) + WS6 (sms-mirror), PLAN-0915 phase 5.
--
-- Adds the "team" audience to the existing action-event bus so
-- application/tour/work-order/lease/payment events can post one rendered
-- copy into the manager's Team thread (`team-comms.server.ts`,
-- `portal_inbox_thread_records.thread_type = 'team'` — no schema change
-- needed there, `thread_type` is free text), and adds a durable
-- `draft_for_review` flag so a retried draft-for-review delivery re-queues
-- the draft instead of falling through to a real send (see
-- `retryDueActionEventDeliveries` in `src/lib/action-events.server.ts`).
--
-- The `audience` check constraint was created on `work_order_event_deliveries`
-- (`20260904130000_work_order_events.sql`) and survived the later rename to
-- `action_event_deliveries` (`20260904140000_action_event_bus.sql`) under its
-- original auto-generated name — Postgres does not rename constraints when a
-- table is renamed. Both candidate names are dropped defensively.

alter table public.action_event_deliveries
  drop constraint if exists work_order_event_deliveries_audience_check,
  drop constraint if exists action_event_deliveries_audience_check,
  add constraint action_event_deliveries_audience_check
    check (audience in ('manager', 'resident', 'vendor', 'team'));

alter table public.action_event_deliveries
  add column if not exists draft_for_review boolean not null default false;

comment on column public.action_event_deliveries.draft_for_review is
  'WS5: true when this delivery was queued as a manager-approval draft (see queueActionEventDraftForReview) rather than sent. Persisted so a retry of a FAILED draft-queue attempt re-queues the draft instead of sending for real.';

-- Extend the atomic lease-transition RPC's audience whitelist so `lease_sent`
-- can carry a `team` delivery alongside the existing manager/resident pair.
-- A team delivery's `recipientKey`/`recipientUserId` must be the manager
-- (the owning account the Team thread belongs to) — the same identity check
-- already applied to a `manager` delivery.
  ...
  if p_event is not null and p_event <> 'null'::jsonb then
    ...
    for v_delivery in select value from jsonb_array_elements(p_event->'deliveries') loop
      if v_delivery->>'audience' is null
         or v_delivery->>'audience' not in ('manager','resident','team')
         ...
         or (v_delivery->>'audience' = 'team' and
             (v_delivery->>'recipientUserId' is distinct from v_manager::text or v_delivery->>'recipientKey' is distinct from v_manager::text))
...
comment on function public.persist_lease_with_action_event(jsonb,timestamptz,jsonb) is
  'Service-only CAS lease persistence plus durable action-event enqueue. Performs no external delivery. WS5: audience now also accepts team (validated against the manager, like the manager audience).';

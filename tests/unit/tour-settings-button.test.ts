...
describe("tour settings entry point", () => {
  it("shows Settings on the manager availability view and opens the scoped Tours tab", () => {
    // The save-refresh callback (`onCalendarSettingsSaved`) is gone: the modal is
    // opened `scoped` on `initialTab="tours"` and the panel owns its own save.
    expect(SRC).toContain('data-attr="calendar-settings-open"');
    expect(SRC).toMatch(/calendarSettingsButton[\s\S]*availabilityView/);
    expect(SRC).toContain('initialTab="tours"');
    expect(SRC).toContain('scopedTitle="Tours"');

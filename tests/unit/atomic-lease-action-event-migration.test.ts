...
const deferredSmsSql = readFileSync(join(process.cwd(), "supabase/migrations/20260909110000_action_event_sms_deferred_until.sql"), "utf8");
...
describe("atomic lease action-event migration", () => {
  ...
  it("adds an explicit timestamp for preserving the independent quiet-hours SMS clock", () => {
    expect(deferredSmsSql).toMatch(/add column if not exists sms_deferred_until timestamptz/i);
  });
});

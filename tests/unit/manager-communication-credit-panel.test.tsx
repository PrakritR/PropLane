...
describe("communication credit purchase states",()=>{
 it("keeps manual top-ups available at zero credit",async()=>{render(<ManagerCommsBillingPanel/>);const buy=await screen.findByRole("button",{name:"Buy credit"});expect((buy as HTMLButtonElement).disabled).toBe(false);fireEvent.click(buy);expect(await screen.findByRole("dialog",{name:"Buy credit"})).toBeTruthy();expect(screen.getByText("Total")).toBeTruthy();});
 it("does not present purchases on an unreadable balance",async()=>{vi.stubGlobal("fetch",vi.fn(async()=>Response.json({error:"Unavailable"},{status:503})));render(<ManagerCommsBillingPanel/>);expect(await screen.findByRole("alert")).toBeTruthy();expect(screen.queryByRole("button",{name:"Buy credit"})).toBeNull();expect(screen.getByRole("button",{name:"Try again"})).toBeTruthy();});
 ...
 it("keeps shared credit visible on native without a Stripe purchase link",async()=>{native.value=true;render(<ManagerCommsBillingPanel/>);expect(await screen.findByText("Remaining")).toBeTruthy();expect(screen.queryByRole("button",{name:"Buy credit"})).toBeNull();expect(screen.queryByRole("link",{name:/buy|purchase/i})).toBeNull();expect(screen.queryByText(/not available in this app/i)).toBeNull();});
 it("hides Buy credit when purchases are paused",async()=>{vi.stubGlobal("fetch",vi.fn(async()=>Response.json({...summary,paygEnabled:false,wallet:{...summary.wallet,remainingCents:14800,includedRemainingCents:14800}})));render(<ManagerCommsBillingPanel/>);expect(await screen.findByText("Purchases paused")).toBeTruthy();expect(screen.queryByRole("button",{name:"Buy credit"})).toBeNull();});
});

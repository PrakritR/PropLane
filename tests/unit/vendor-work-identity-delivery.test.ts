...
const provider = { email: vi.fn().mockResolvedValue({ id: "email-1" }), sms: vi.fn().mockResolvedValue({ id: "sms-1" }) };
function db(opts: { ready?: boolean; claimed?: boolean; cap?: boolean } = {}) {
  const writes: unknown[] = [];
  const from = vi.fn((table: string) => { const q: Record<string, unknown> = {}; q.select=()=>q; q.eq=()=>q; q.update=(v:unknown)=>{writes.push({table,v});return q}; q.maybeSingle=async()=>({data:opts.ready===false?null:{id:"i",email_address:"vendor@prop.test",phone_number:"+12065550111",email_state:"ready",sms_state:"ready",email_send_ready:true,sms_send_ready:true,email_domain_verified:true},error:null}); q.then=(r:(v:unknown)=>unknown)=>r({error:null}); return q; });
...
const base={vendorUserId:"v",channel:"email" as const,recipient:"r@test",subject:"Hi",text:"Body",idempotencyKey:"k",runtimeEnabled:true,providerConfigured:true};
beforeEach(()=>{vi.clearAllMocks(); suppression.mockResolvedValue({ok:true,optedOut:false});});
describe("vendor sponsored delivery",()=>{
 it.each([[false,true,"provider_disabled"],[true,false,"provider_unconfigured"]] as const)("blocks unavailable provider without send",async(runtimeEnabled,providerConfigured,reason)=>{const x=db();expect((await deliverVendorWorkIdentity(x.db,{...base,runtimeEnabled,providerConfigured},provider)).reason).toBe(reason);expect(provider.email).not.toHaveBeenCalled();});

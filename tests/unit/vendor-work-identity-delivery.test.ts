...
function db(opts: { ready?: boolean; runtime?: boolean; claimed?: boolean; cap?: boolean; replay?: "sent" | "reconciling"; mismatch?: boolean } = {}) {
  const writes: unknown[] = [];
  const from = vi.fn((table: string) => { const q: Record<string, unknown> = {}; let selected=""; q.select=(s:string)=>{selected=s;return q}; q.eq=()=>q; q.insert=(v:unknown)=>{writes.push({table,v});return q}; q.update=(v:unknown)=>{writes.push({table,v});return q}; q.maybeSingle=async()=>({data:table==="vendor_work_identity_runtime"?{enabled:opts.runtime??true}:table==="vendor_work_identity_delivery_attempts"?{id:"attempt"}:table==="vendor_work_identity_outbox"&&selected.includes("status")?(opts.replay?{status:opts.replay,provider_message_id:"prior",recipient:opts.mismatch?"other@test.com":"r@test.com",context_fingerprint:opts.mismatch?"other":"recipient:r@test.com|subject:Hi|body:Body",subject:"Hi",body:"Body",channel:"email"}:null):opts.ready===false?null:{id:"i",email_address:"vendor@prop.test",phone_number:"+12065550111",email_state:"ready",sms_state:"ready",email_send_ready:true,sms_send_ready:true,email_domain_verified:true},error:null}); q.then=(r:(v:unknown)=>unknown)=>r({error:null}); return q; });
  const rpc = vi.fn((name:string) => Promise.resolve(name.includes("operation") ? {data:{operation_id:"op",claimed:opts.claimed??true},error:null} : opts.cap ? {data:{outbox_id:null,blocked_reason:"platform_cap_reached"},error:null} : {data:{outbox_id:"out",claimed:opts.replay?false:opts.claimed??true},error:null}));
...
describe("vendor sponsored delivery",()=>{
 ...
 it("holds reconciling replays and rejects a changed payload",async()=>{const pending=db({replay:"reconciling"});expect((await deliverVendorWorkIdentity(pending.db,base,provider)).reason).toBe("provider_outcome_unknown");const mismatch=db({replay:"sent",mismatch:true});expect((await deliverVendorWorkIdentity(mismatch.db,base,provider)).reason).toBe("idempotency_mismatch");expect(provider.email).not.toHaveBeenCalled();});

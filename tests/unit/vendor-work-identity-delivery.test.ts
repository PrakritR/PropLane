...
function db(opts: { ready?: boolean; claimed?: boolean; cap?: boolean } = {}) {
  ...
  const from = vi.fn((table: string) => { const q: Record<string, unknown> = {}; q.select=()=>q; q.eq=()=>q; q.update=(v:unknown)=>{writes.push({table,v});return q}; q.maybeSingle=async()=>({data:table==="vendor_work_identity_runtime"?{enabled:true}:opts.ready===false?null:{id:"i",email_address:"vendor@prop.test",phone_number:"+12065550111",email_state:"ready",sms_state:"ready",email_send_ready:true,sms_send_ready:true,email_domain_verified:true},error:null}); q.then=(r:(v:unknown)=>unknown)=>r({error:null}); return q; });

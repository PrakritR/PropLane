...
describe("vendor identity effective readiness",()=>{
 ...
 it("keeps email directions independent",()=>{const r=responseFor({identity:{...identity,email_receive_ready:false,email_send_ready:true},runtime,emailConfigured:true,smsConfigured:true,outboundUsed:0});expect(r.email).toMatchObject({sendReady:true,receiveReady:false});});
 it("keeps attached inbound SMS available while carrier registration blocks sending",()=>{const r=responseFor({identity:{...identity,carrier_ready:false,sms_send_ready:false,sms_receive_ready:true},runtime,emailConfigured:true,smsConfigured:true,outboundUsed:0});expect(r.sms).toMatchObject({sendReady:false,receiveReady:true});});
});

...
describe("Communication nav badge counts what the conversation list shows", () => {
  ...
  it("hides an inbound-SMS notice from the badge once the SMS Communication UI is enabled", () => {
    inboxState.rows = [SMS_NOTICE_UNREAD];
    const { result: hidden } = renderHook(() => usePortalNavCounts("manager", true));
    expect(hidden.current.communication?.count).toBe(0);
...
describe("Communication nav badge folds in SMS conversations once the SMS UI is enabled", () => {
  ...
  it("ignores SMS conversations entirely when the SMS Communication UI is off", async () => {
    inboxState.rows = [];
    smsConversationsState.residents = [
      smsResident({ conversationKey: "conv-3", name: "Sam Resident", messages: [inboundSmsMessage("m4")] }),
    ];
    const { result } = renderHook(() => usePortalNavCounts("manager", false));
    // No fetch happens at all when the SMS UI is off; give any stray
    // microtask a turn before asserting the conversation never counts.
    await act(async () => {
...
describe("countUnreadActiveConversations (pure)", () => {
  ...
  it("ignores every SMS conversation when smsUiEnabled is false", () => {
    const residents = [

...
const state = vi.hoisted(() => ({
  linkedManagers: [] as { managerUserId: string; label: string }[],
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url: unknown, init?: { method?: string; body?: string }) => {
      ...
      return {
        ok: true,
        status: 200,
        json: async () => ({ invoices: [], linkedManagers: state.linkedManagers }),
...
describe("vendor invoice manager picker", () => {
  ...
  it("does not ask when there is only one manager to bill", async () => {
    state.linkedManagers = [{ managerUserId: "mgr-a", label: "Alex Manager" }];
    renderInvoices();
    await openSubmitModal();

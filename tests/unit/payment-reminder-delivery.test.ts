...
describe("deliverPaymentReminder", () => {
  beforeEach(() => {
    ...
    vi.mocked(sendResidentOutboundSms).mockResolvedValue({ sent: true, channel: "managed" });
    vi.stubGlobal(
      "fetch",
  ...
  it("resumes only a confirmed failed SMS channel after email and inbox submitted", async () => {
    ...
    vi.mocked(sendResidentOutboundSms)
    ...
    expect(sendResidentOutboundSms).toHaveBeenCalledTimes(2);

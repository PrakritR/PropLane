...
export async function mintInviteLink(
  ...
  // person who lives in the property, not a seat on the manager's plan, so
  ...
  // A resident link grants no modules, so it stores an empty permission map
  // rather than a normalized co-manager one. Nothing reads it on the resident
  // redeem path, and leaving a populated map on the row would be a grant
  // waiting for a future reader to honour.
  const permissions =
    kind === "resident" ? {} : normalizePropertyCoManagerPermissions(input.propertyPermissions, propertyIds);

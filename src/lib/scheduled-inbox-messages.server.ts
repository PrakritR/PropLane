...
export async function loadDueScheduledInboxMessages(db: SupabaseClient, now = new Date()) {
  const { data, error } = await db.from("portal_scheduled_inbox_message_records").select(SELECT).in("status", ["scheduled", "sending"]).is("row_data->>smsTestSessionId", null).lte("send_at", now.toISOString()).order("send_at", { ascending: true }).limit(200);
  if (error) throw error; return mapRows(data as DbRow[] | null);
}

/** Read the row after its first channel claim has frozen its payload. */
export async function loadScheduledInboxMessageForDelivery(db: SupabaseClient, managerUserId: string, id: string) {
  const { data, error } = await db.from("portal_scheduled_inbox_message_records").select(SELECT).eq("id", id).eq("manager_user_id", managerUserId).maybeSingle();
  if (error) throw error; return data ? rowFromDb(data as DbRow) : null;
}

...
- Room pricingMode: "fixed" means the listed priceLabel is locked — if they ask for a different amount, say the price is set and escalate only if they still need the manager. "flexible" means the listed priceLabel is the asking rent but a counter-offer is allowed — escalate_to_manager so the manager can approve or decline; never invent a new number yourself.

 ...
 *    manager typed to their MOST RECENT resident thread, with no way to see or
 *    choose which thread that was — and manager-cell forwarding is disabled in
 *    the managed-number regime, so they had not even been shown the message
 *    they were replying to. Contacting a resident is now an explicit proposal
 *    (`send_message` / `reply_to_thread`) naming the recipient, confirmed by YES.

...
export async function sweepTaskOverdue(db: SupabaseClient, now: Date = new Date()): Promise<number> {
  ...
  const entries: { managerUserId: string; task: Record<string, unknown>; propertyId: string | null }[] = [];
  for (const row of data ?? []) {
    ...
    for (const task of tasks as Record<string, unknown>[]) {
      ...
      const propertyId = typeof task.propertyId === "string" && task.propertyId.trim() ? task.propertyId.trim() : null;
      entries.push({ managerUserId, task, propertyId });
    }
  }
  ...
  let queued = 0;
  for (const { managerUserId, task, propertyId } of entries) {
...
export async function sweepDocumentSignatureReminders(db: SupabaseClient, now: Date = new Date()): Promise<number> {
  ...
  const { data, error } = await db
    ...
    .select("id, manager_user_id, display_name, resident_user_id, resident_email, signature_requested_at, property_id")
  ...
  const reminderResolver = await loadReminderSettingsResolver(db, rows.map((row) => String(row.manager_user_id)));
...
export async function sweepResidentWelcome(db: SupabaseClient, now: Date = new Date()): Promise<number> {
  ...
  if (rows.length === 0) return 0;
  const reminderResolver = await loadReminderSettingsResolver(db, rows.map((row) => String(row.manager_user_id)));
  const rowPropertyId = (row: (typeof rows)[number]): string | null => {
    const appPropertyId = (row.row_data as { propertyId?: unknown } | null)?.propertyId;
    if (typeof appPropertyId === "string" && appPropertyId.trim()) return appPropertyId.trim();
    return typeof row.property_id === "string" && row.property_id.trim() ? row.property_id.trim() : null;
  };
  // A house's own reminder override wins when it has one (PLAN-0916-1040).
  const active = rows.filter((row) => reminderResolver.resolve(String(row.manager_user_id), rowPropertyId(row)).rules.resident_welcome.enabled);
  ...
  for (const row of active) {
    const managerUserId = String(row.manager_user_id);
    const settings = reminderResolver.resolve(managerUserId, rowPropertyId(row));

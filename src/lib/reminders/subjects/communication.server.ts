...
export async function sweepUnansweredMessages(db: SupabaseClient, now: Date = new Date()): Promise<number> {
  ...
  // An inbox thread has no property, so `message_unanswered` always resolves
  // to the workspace rule (PLAN-0916-1040) — no per-house override ever
  // applies here, so the plain workspace-only load is enough; the resolver's
  // extra property-override query would be paid for and never read.

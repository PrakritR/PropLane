...
/**
 * What is offered for a property:
 *
...
/**
 * The public route is unauthenticated and `no-store`, and each request fans out
 * one Google Calendar read per host manager — a call that can also refresh and
 * write back the manager's OAuth token. The in-process busy cache blunts repeat
 * load only per instance, which on a serverless platform is a weak throttle, so
 * the REQUEST is capped per IP by the route. Exported so the cap lives with the
 * reasoning for it rather than drifting in the handler.
 */
export const TOUR_AVAILABILITY_RATE_LIMIT = 60;
export const TOUR_AVAILABILITY_RATE_LIMIT_WINDOW_MS = 60_000;
/**
...
/** The manager(s) who can host a given slot, keyed by `slotKey`. */
...
/**
 * Every tour slot currently on offer for a property, and who can host each one.
 *
 * This is THE answer to "what is open", shared by the public booking grid and
 * every agent tour tool. It was the body of
 * `GET /api/public/property-tour-availability` and moved here so a tool would
 * not have to `fetch()` an internal route (docs/ai-assistant.md) — and, more
 * importantly, so nothing can offer a slot the public grid would not.
 *
 * That mattered immediately: `loadManagerTourBlocks` (tour-proposal.server.ts)
 * only *mirrors* this exclusion set by hand and omits Google-busy entirely, so
 * an agent computing availability from it would have handed out times the
 * manager's linked calendar says they are busy.
 */
export async function listOpenTourSlots(
  ...
  try {
    ...
    const slotHosts: PropertyTourSlotHosts = {};

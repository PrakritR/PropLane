...
export async function deliverVendorWorkIdentity(
  db: SupabaseClient,
  input: { vendorUserId: string; channel: VendorIdentityChannel; recipient: string; recipientUserId?: string | null; subject: string; text: string; idempotencyKey: string; runtimeEnabled: boolean; providerConfigured: boolean },
  provider: VendorDeliveryProvider,
): Promise<{ ok: boolean; sent?: boolean; reason?: string; providerMessageId?: string | null }> {
  if (!input.runtimeEnabled) return { ok: false, reason: "provider_disabled" };
  if (!input.providerConfigured) return { ok: false, reason: "provider_unconfigured" };
  if (!input.recipient.trim() || !input.idempotencyKey.trim()) return { ok: false, reason: "invalid_recipient" };
  ...
  if (!email) {
    const suppression = await readSmsSuppressionState(db, input.recipient, { userId: input.recipientUserId ?? null });

...
export async function GET() {
  try {
    ...
    for (let chunk = 0; chunk < ids.length; chunk += 100) {
      for (const column of ["row_data->>assignedPropertyId", "row_data->>propertyId", "row_data->application->>propertyId"]) {
        for (let offset = 0; ; offset += 500) {
          const { data, error } = await db.from("manager_application_records")
            ...
            .eq("row_data->>bucket", "approved").in(column, ids.slice(chunk, chunk + 100)).order("id").range(offset, offset + 499);

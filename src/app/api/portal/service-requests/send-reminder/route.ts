...
import { deliverResidentServiceRequestReminder } from "@/lib/resident-service-request-reminder.server";
...
/** Resident follow-up on a pending add-on service request — inbox + email to property managers. */
export async function POST(req: Request) {
  try {
    ...
    const result = await deliverResidentServiceRequestReminder(db, {
      requestId,
    ...
    track("resident_service_request_reminder_sent", actor.userId, {

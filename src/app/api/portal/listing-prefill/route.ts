/**
 * `POST /api/portal/listing-prefill` — facts and a rent estimate for one
 * address. Manager-only.
 *
...
export async function POST(req: Request) {
  ...
  if (cached) {
    track("listing_prefill_looked_up", ctx.userId, { found: Boolean(cached.facts), cached: true });
    return answer({ status: cached.facts ? "found" : "none", facts: cached.facts, rent: cached.rent, cached: true, source: cached.source });
  }

  const quota = await consumeLookup(db, ctx.userId);
  if (!quota.ok) {
    if (quota.reason === "quota") {
      track("listing_prefill_looked_up", ctx.userId, { found: false, cached: false, quota: true });
  ...
  try {
    const lookup = await lookupAddressFacts(input);
    await writePrefillCache(db, key, { facts: lookup.facts, rent: lookup.rent, source: provider });
    track("listing_prefill_looked_up", ctx.userId, { found: Boolean(lookup.facts), cached: false });
    return answer({

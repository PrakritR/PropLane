...
/**
 * A scaled, non-interactive slice of the REAL `/demo` portal — not a
 * hand-drawn replica of it (captain 2026-09-25: "NOTHING like the real
 * portal"). `/demo?role=&section=` deep-links straight into one view
 * (`demo-manager-shell.tsx`/`page.tsx`); the iframe is a genuinely separate
 * browsing context sized to the card, so the app's own responsive layout
 * renders at that width — no fixed "phone" viewport needed, unlike the hero
 * window's phone mockup. `pointer-events-none` plus `tabIndex={-1}` keep it
 * a picture, not a second interactive copy of the portal sitting mid-page.
 */
function DemoRouteSlice({ role, section, height, label }: { role: DemoPortalRole; section: string; height: number; label: string }) {
  return (
    <div

...
import { commitProspect, commitResident, sendClosingMessage, type CommitOutcome } from "./commit";
...
export function AddResidentWizard({
  ...
  const buildRow = useCallback(() => {
    ...
    return form.kind === "prospect"
      ? buildProspectRow(form, ctx)
      : buildManualResidentRow(form, ctx, derived.customQuestions.map((q) => ({ key: q.key, label: q.label, type: q.type, section: q.section })));
  }, [form, managerUserId, propertyOptions, derived.customQuestions]);
  ...
  const railHeader = (
    <button
      type="button"
      onClick={() => goTo(form.kind === "prospect" ? "contact" : "documents")}
    ...
    >
      <span className="flex flex-col items-center gap-1.5">
        ...
        {form.documents.length ? `${form.documents.length} ${form.documents.length === 1 ? "document" : "documents"}` : form.kind === "prospect" ? "Contact and tour" : "Add documents"}
      </span>
    </button>
  );
  ...
  return (
    <>
      ...
      >
        ...
        {stepId === "review" ? <ReviewStep form={form} patch={patch} derived={derived} propertyLabel={propertyLabel} goTo={goTo} /> : null}
      </AddWorkspace>
      {preview ? (

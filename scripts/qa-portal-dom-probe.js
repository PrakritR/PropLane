...
(() => {
  ...
  globalThis.__tmProbe = function __tmProbe(opts) {
    const skip = new Set((opts && opts.skip) || []);
    const vw = document.documentElement.clientWidth;
    const vh = document.documentElement.clientHeight;
    const findings = [];
    const runSkip = (name, fn) => run(name, fn, skip);
    ...
    findings.push(...runSkip("page-overflow-x", () => {
    ...
    findings.push(...runSkip("unreachable-overflow", () => {
      const out = [];
      for (const el of all) {
    ...
    findings.push(...runSkip("obscured-control", () => {
      const out = [];
    ...
    findings.push(...runSkip("duplicate-control", () => {
      const seen = new Map();
    ...
    findings.push(...runSkip("ambiguous-repeated-button", () => {
    ...
    findings.push(...runSkip("tiny-tap-target", () => {
      if (vw > 500) return [];
      const out = [];
      for (const el of Array.from(document.querySelectorAll(INTERACTIVE))) {
        if (!visible(el)) continue;
    ...
    findings.push(...runSkip("offscreen-left", () => {
      const out = [];
      for (const el of all) {
        const s = getComputedStyle(el);
    ...
    findings.push(...runSkip("broken-image", () => {
    ...
    findings.push(...runSkip("clipped-text", () => {
      const out = [];
      for (const el of all) {
    ...
    findings.push(...runSkip("no-heading", () => {
    ...
    findings.push(...runSkip("error-surface", () => {
      const body = text(document.body);
      const patterns = [

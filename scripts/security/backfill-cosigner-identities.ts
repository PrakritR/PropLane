...
async function main() {
  ...
  try {
    ...
    const rows = await client.query(`select id, manager_user_id, row_data from public.cosigner_submission_records${apply ? " for update" : ""}`);
    let changedRows = 0;
    let plaintextFields = 0;
    for (const row of rows.rows) {
      const result = protectCosignerRecord(row.row_data, row.id, row.manager_user_id);
      plaintextFields += result.plaintextFields;
      if (!result.changed) continue;
      changedRows++;
      if (apply) await client.query("update public.cosigner_submission_records set row_data = $1::jsonb, updated_at = now() where id = $2", [JSON.stringify(result.value), row.id]);
    }
    ...
    throw new Error("Co-signer backfill failed and rolled back.");
  } finally {
    await client.end();
  }
}
...
main().catch(() => {
  console.error("Co-signer backfill failed. Verify target, trusted TLS, row ownership and encryption keys without logging sensitive values.");
  process.exitCode = 1;
});

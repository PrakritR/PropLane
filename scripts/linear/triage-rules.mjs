...
const HAPPY_PATH_BREAK = /\b(crash|404|blocked|redirect|sign-?up|sign-?in|login|onboarding|apply|payment|pay\b|stuck|spinner|error|regression|data loss|cannot|can't|won't|broken)\b/i;
...
const LOW_SURFACE = /\b(typo|polish|marketing|admin-only|dev tooling|qa epic|coordination|localstorage|minor)\b/i;
...
export function inferPriority(issue) {
  ...
  if (HAPPY_PATH_BREAK.test(text) && /auth|portal|resident|manager/i.test(text)) return PRIORITY.high;

// usage: node .shot.tmp.cjs <role:manager|resident|vendor> <route...>
const{chromium}=require('@playwright/test');const fs=require('fs');
const OUT='/private/tmp/claude-501/-Users-prakrit-firstmate-projects-proplane-claude-3/465f868e-c1f2-459a-807b-be3cd898b80f/scratchpad/shots/';
const STATE='/private/tmp/claude-501/-Users-prakrit-firstmate-projects-proplane-claude-3/465f868e-c1f2-459a-807b-be3cd898b80f/scratchpad/state-';
const BASE='http://localhost:3003';
const ACC={manager:['manager@test.proplane.local','TestManager123!','Property','/portal/dashboard'],resident:['resident@test.proplane.local','TestResident123!','Resident','/resident/dashboard'],vendor:['vendor@test.proplane.local','TestVendor123!','Vendor','/vendor/dashboard']};
async function login(p,role){const[email,pw,label,next]=ACC[role];await p.goto(BASE+'/auth/sign-in?next='+encodeURIComponent(next),{waitUntil:'domcontentloaded'});
 if(p.url().includes('/auth/sign-in')){await p.getByPlaceholder('Email').fill(email);await p.getByPlaceholder('Password').fill(pw);await p.getByRole('button',{name:/sign in/i}).click();await p.waitForURL(u=>!u.pathname.includes('/auth/sign-in'),{timeout:60000});}
 if(new URL(p.url()).pathname.startsWith(next))return;
 await p.goto(BASE+'/auth/choose-portal?next='+encodeURIComponent(next),{waitUntil:'domcontentloaded'});
 const opt=p.getByRole('button',{name:new RegExp('^'+label)});
 for(let i=0;i<40;i++){if(new URL(p.url()).pathname.startsWith(next))return;if(await opt.count()&&await opt.first().isEnabled().catch(()=>false)){await opt.first().click();break}await p.waitForTimeout(500)}
 await p.waitForURL(u=>u.pathname.startsWith(next),{timeout:45000});}
(async()=>{const[role,...routes]=process.argv.slice(2);const b=await chromium.launch();const stateFile=STATE+role+'.json';
const ctx=await b.newContext({viewport:{width:1440,height:900},...(fs.existsSync(stateFile)?{storageState:stateFile}:{})});const p=await ctx.newPage();
p.on('pageerror',e=>console.log('PAGEERR',e.message.slice(0,160)));
await login(p,role);await ctx.storageState({path:stateFile});
for(const r of routes){const[route,tag]=r.split('@');try{await p.setViewportSize({width:1440,height:900});await p.goto(BASE+route,{waitUntil:'domcontentloaded',timeout:60000});await p.waitForTimeout(2500);const name=(tag||route.replace(/^\//,'').replace(/[\/?=&]/g,'-'));await p.screenshot({path:OUT+name+'-desktop.png'});await p.setViewportSize({width:390,height:844});await p.waitForTimeout(800);await p.screenshot({path:OUT+name+'-mobile.png'});console.log('ok',route,'->',new URL(p.url()).pathname,name)}catch(e){console.log('fail',route,e.message.slice(0,100))}}
await b.close();})().catch(e=>{console.error(e);process.exit(1)});

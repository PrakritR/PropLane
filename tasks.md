...
- [PRP-184 — Website UI audit findings](https://linear.app/axishousing/issue/PRP-184/website-ui-audit-aug-2026-11-open-findings-incl-19-colour-contrast)
  The August UI audit contains 11 unresolved findings, including 19 contrast failures, nested interactive controls, and a soft 404. **All 4 P1 findings done September 4, 2026** (commits `f213d44e`, `3754767a`, `9e233f3b`, `a6d60599` on `main`; #11 was already fixed independently upstream, verified rather than redone):
  - **#11 soft-404** — verified against the upstream fix (part of a broader "Communication is one inbox" redesign); my own duplicate attempt was discarded.
  - **#1 nested interactive controls** — `resident-dashboard.tsx` was already fixed upstream; fixed the remaining piece, `DataListDesktopRow` (shared by Tours and every `DataList` consumer) — row activation is now a real `<button>`, sibling to the checkbox/actions, never nested.
  - **#2 contrast** — fixed two root-caused CSS bugs (the auth "legibility wash" gradient was backwards, and the OpsSky tab strip needed a scrim, not a color bump). A separate, much larger finding — `--pl-blue` (`#2f6bff`) failing WCAG AA as small text/button fills in **14** places site-wide — was evaluated two ways (darken the token; size the text into WCAG's large-text exception) and the captain declined both: the existing contrast reads fine in practice. `scripts/audit-contrast.mjs` now tracks those 14 as explicit, dated exceptions rather than silently passing or failing — it still fails loud on the 3 unrelated failures (an amber tone, a lighter link-hover blue) that were never reviewed.
  - **#4 scroll keyboard access** — added `tabIndex`/`role="region"` to all 3 identified scroll containers.
  - **P2 (findings 5-10) remains untouched** — the ticket's own text says this set should be planned as one redesign pass, not picked off individually.
